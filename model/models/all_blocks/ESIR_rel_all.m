...
function E = ESIR_rel_all(p,tc,xd,x0,N,vectorIni)
% global nTau 
global h Mv contF
nTau = 6;
% xd
...
options = ddeset('RelTol',1e-2,'AbsTol',1e-4,...
                 'InitialY',vectorInicial,'NormControl','on','InitialStep',100);
...
E= [  ( alfa.*y(2,:)-xd(:,1)' )./( alfa.*y(2,:))   ( y(4,:)-xd(:,3)' )./y(4,:) ];
% E= [ ( alfa.*y(2,:)-xd(:,1)' )./(alfa.*y(2,:))   ( y(4,:)-xd(:,3)' )./y(4,:) ( y(3,:)-xd(:,2)' )./y(3,:) ( y(1,:)-xd(:,4)' )./y(1,:) ];
