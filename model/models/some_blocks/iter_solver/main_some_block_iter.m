...
% Para error relativo
tol=1e-1;
[pint,rint,~,~,~,~,~]=lsqnonlin(@(p) ESIR_some(p,tc,Data,x0,N),p0,Lb,Ub,options);
er=2*tol;
maxit=10;
it=0;
while (er>tol)&&(it<maxit)
    [p,r,~,~,~,~,jac]=lsqnonlin(@(p) ESIRw_some(p,tc,Data,x0,N,pint),pint,Lb,Ub,options);
    er=norm(p-pint)/norm(p);
    pint=p;
    it=it+1;
end

%[x,fval,exitflag,output] = fminsearch(ESIR(p,tc,Data,x0,N),p0,options)
% %% Considerando beta variable hasta el 27 de abril o 02 de sept.
%disp('p=[beta1;beta2;beta3;gamma;tau1;tau2;a;k;aC]:');
% %% Considerando beta variable hasta el 16 de junio
% disp('p=[beta1;beta2;beta3;beta4;beta5;gamma;tau1;tau2;a;k;aC]:');





%% Considerando beta variable hasta el 07 de sept. en RM
%disp('p=[beta1;beta2;beta3;beta4;beta5;beta6;gamma;tau1;tau2;a;k;aC]:');
format short e
